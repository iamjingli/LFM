...
def get_pseudo_labels(model, test_dataloader, k_idx, unk_idx, clip_label, args, clip_max_s):
    ...
    #     f.write(f"LFM聚类时间 数据集：{args.dataset}, 任务：{args.s_idx}->{args.t_idx} 时间(ms)：{t:.0f}\n")
    # import sys 
    # sys.exit(0)
...
def get_idx(args, dataloader):
    ...
    # t = (end - start) * 1000
    # with open("TMM_MR/time.txt", "a+") as f:
    #     f.write(f"CLIP推理时间 数据集：{args.dataset}, 任务：{args.s_idx}->{args.t_idx} 时间(ms)：{t:.0f}\n")
...
if __name__ == "__main__":
    ...
    t = (end - start) * 1000
    with open("TMM_MR/time.txt", "a+") as f:
        f.write(f"完整时间 数据集：{ a.dataset}, 任务：{ a.s_idx}->{a.t_idx} 时间(ms)：{t:.0f}\n")
