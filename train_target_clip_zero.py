...
def get_pseudo_labels(model, test_dataloader, k_idx, unk_idx, clip_label, args, clip_max_s):
    ...
    soft_acc = (soft_label == gt_label_bank[k_idx]).sum() / len(k_idx)
...
def get_idx(args, dataloader):
    ...
    clip_zero_shot_labels = [item.lower() for item in args.test_labels]
    clip_zero_shot_labels.append("unknown")
    _, clip_label, _, _ = get_ood_scores_clip(args, net, dataloader, llm_labels)
    import sys
    sys.exit(0)
    ...
    for _, _, imgs_label, _ in dataloader:
        gt_label_stack.append(imgs_label)

    gt_label_all = torch.cat(gt_label_stack, dim=0)  # [N]
    pred_cls_all = clip_label # [N, C]
    # with open("TMM_MR/clip_zero.txt", "a") as f:
    #     if  args.s_idx == 0 and args.t_idx == 1:
    #         f.write(f"\ntarget:{args.target_label_type}\n")
    #     f.write(str(gt_label_all.tolist()) + "\n")
    #     f.write(str(pred_cls_all.argmax(1).tolist()) + "\n")
    h_score, known_acc, unknown_acc, _ = compute_h_score(args, class_list, gt_label_all, pred_cls_all, args.target_private_class_num > 0,
                                                         open_thresh=args.w_0)
    log = f"known_acc:{known_acc:.3f} unknown_acc:{unknown_acc:.3f} h-score:{h_score:.3f}\n"
    with open("TMM_MR/clip_zero.txt", "a") as f:
        if  args.s_idx == 0 and args.t_idx == 1:
            f.write(f"\ntarget:{args.target_label_type} --index:{args.index}\n")
        f.write(log)

    ## 直接用unknown
    # gt_label_stack = []
    # class_list = []
    # open_flag = args.target_private_class_num > 0
    # if(open_flag):
    #     class_list = [i for i in range(args.source_class_num + 1)]
    # else:
    #     class_list = [i for i in range(args.source_class_num)]

    # for _, _, imgs_label, _ in dataloader:
    #     gt_label_stack.append(imgs_label)

    # gt_label_all = torch.cat(gt_label_stack, dim=0)  # [N]
    # clip_label = clip_label[0]  # [N]
    # with open("TMM_MR/clip_zero.txt", "a") as f:
    #     if  args.s_idx == 0 and args.t_idx == 1:
    #         f.write(f"\ntarget:{args.target_label_type}\n")
    #     f.write(str(gt_label_all.tolist()) + "\n")
    #     f.write(str(clip_label.tolist()) + "\n")
    # print(gt_label_all.shape)
    # print(clip_label.shape)
    # import sys
    # sys.exit(0)

    # per_class_num = torch.zeros((len(class_list)))
    # per_class_correct = torch.zeros_like(per_class_num)

    # for i, label in enumerate(class_list):
    #     label_idx = torch.where(gt_label_all == label)[0]
    #     correct_idx = torch.where(clip_label[label_idx] == label)[0]
    ...
    #     print(f"known_acc:{known_acc.item():.2f} unknown_acc:{unknown_acc.item():.2f} h-score:{h_score.item():.2f}\n")
    #     log += f"known_acc:{known_acc.item():.2f} unknown_acc:{unknown_acc.item():.2f} h-score:{h_score.item():.2f}\n"
    # else:
    #     known_acc = per_class_correct.sum() / (per_class_num.sum() + 1e-5)
    #     unknown_acc = 0.0
    #     h_score = 0.0
    #     print("known_acc:", known_acc.item())
    #     log += f"known_acc: {known_acc.item()}\n"
...
def main(args):
    ...
    model_save_dir = os.path.join(this_dir, "checkpoints", "model", args.dataset, "s_{}_t_{}".format(args.s_idx, args.t_idx),
    ...
    for epoch_idx in range(args.epochs):
        ...
        if args.target_label_type == 'PDA' or args.target_label_type == 'CLDA':
            if knownacc >= best_known_acc:
                ...
                # checkpoint_file = "{}_checkpoint.pth".format(args.dataset)
                # torch.save({
                #     "epoch": epoch_idx,
                #     "model_state_dict": model.state_dict()}, os.path.join(model_save_dir, checkpoint_file))
        else:
