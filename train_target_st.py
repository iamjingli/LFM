...
def get_pseudo_labels(model, test_dataloader, k_idx, unk_idx, clip_label, args, clip_max_s):
    ...
    args.logger.info(f"clip label acc: {clip_acc:.1%}")
    args.logger.info(f"soft label acc: {soft_acc:.1%}")
...
def tr(args, model, train_dataloader, test_dataloader, optimizer, epoch_idx, k_idx, unk_idx, clip_label, clip_max_s):
    ...
    for img_train, _, _, img_idx in tqdm(train_dataloader, ncols=60):
        ...
        loss_k = torch.sum(- b_pseudo_label[b_k_idx] * torch.log(pred_cls[b_k_idx]), 1).mean()
        loss_total += loss_k

        loss_k_buff.append(loss_k.item())

        loss_unk = torch.sum(pred_cls[b_unk_idx] * torch.log(pred_cls[b_unk_idx]), 1).mean()
        loss_total += loss_unk

        loss_unk_buff.append(loss_unk.item())
        loss_cls_buff.append(loss_total.item())
    ...
    return {"unk": np.mean(loss_unk_buff),
            "k": np.mean(loss_k_buff),
            "all": np.mean(loss_cls_buff)}
...
def main(args):
    ...
    for epoch_idx in range(args.epochs):
        # Train on target
        loss = tr(args, model, target_train_dataloader, target_test_dataloader, optimizer, epoch_idx, k_idx, unk_idx,
        ...
        log = (f"H-Score: {hscore:.3f} k-acc: {knownacc:.3f} unk-acc: {unknownacc:.3f}\t"
               f"loss-- k: {loss['k']:.3f}, unk: {loss['unk']:.3f}, all: {loss['all']:.3f} --\n")

        path = f"stability/{args.s_idx}to{args.t_idx}.txt"
