...
class SFUniDADataset(Dataset):
    ...
    def __init__(self, args, data_dir, data_list, d_type, preload_flg=True) -> None:
        ...
        if args.dataset == "Office" and args.target_label_type == "OSDA":
            self.target_private_classes = [i + args.shared_class_num + args.source_private_class_num + 10 for i in
                                           range(args.target_private_class_num)]
        else:
            self.target_private_classes = [i + args.shared_class_num + args.source_private_class_num for i in
                                           range(args.target_private_class_num)]
    ...
    def pre_loading(self):
        if "Office" in self.dataset and self.preload_flg:
            ...
            self.img_list = [self.resize_trans(Image.open(os.path.join(self.data_dir, item[0])).convert("RGB")) for item
                             in tqdm(self.data_list, ncols=60)]
...
class SFUniDADataset_cam(Dataset):

    def __init__(self, args, data_dir, data_list, d_type, preload_flg=True) -> None:
        super(SFUniDADataset_cam, self).__init__()
        ...
        if self.d_type == "source":
            ...
            self.data_list = [item for item in self.data_list if int(item[1]) in self.shared_classes]
        else:
            # self.data_dir = args.target_data_dir
            self.data_list = [item for item in self.data_list if int(item[1]) in self.target_classes]

        self.path = [os.path.join(self.data_dir, item[0]) for item in self.data_list]
        self.pre_loading()
